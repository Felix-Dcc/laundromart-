...
// NOTE: 'picked_up' removed from filter - providers see orders that are already picked up
// Providers cannot set status to 'picked_up' (only riders can)
const STATUS_OPTIONS = ['', 'pending', 'washing', 'ready', 'delivered', 'cancelled'];

export default function ProviderOrdersScreen({ route, navigation }) {
  ...
  async function loadOrders(p = 1, reset = false) {
    try {
      ...
      const res = await providerAPI.getOrders(params);
...
const styles = StyleSheet.create({
  ...
  searchBtn: { backgroundColor: '#198754', borderRadius: 8, paddingHorizontal: 14, justifyContent: 'center' },
  ...
  reqNum: { fontSize: 15, fontWeight: '700', color: '#198754' },
